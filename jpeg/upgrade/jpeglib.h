...
typedef struct {
  ...
  /* NB: there are additional private fields in an actual quantization data
   * structure, so don't allocate one directly.  Use jpeg_alloc_quant_table.
   */
} JQUANT_TBL;
...
typedef struct {
   ...
   */
  boolean sent_table;		/* TRUE when table has been output */
  /* NB: there are additional private fields in an actual Huffman-table data
   * structure, so don't allocate one directly.  Use jpeg_alloc_huff_table.
   */
} JHUFF_TBL;
...
struct jpeg_decompress_struct {
  ...
  boolean do_fancy_upsampling;	/* T = apply fancy upsampling */
  boolean want_raw_output;      /* T = raw (YCbCr subsampled) output*/
  ...
  struct jpeg_color_quantizer * cquantizer;
};
...
EXTERN JDIMENSION jpeg_read_raw_scanlines JPP((j_decompress_ptr cinfo,
                                      JSAMPIMAGE scanarray,
                                      JDIMENSION max_lines));
